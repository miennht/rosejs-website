...
**Brand / domain:** RoseJS · roseng.org — **`docs/Brand_and_Domain.md`**  
...
| **LinkButton** | Internal `<Link>` or external `<a>` styled as button | `to` _or_ `href`, `variant`, `target`, `rel`, `onClick` |
...
| **FeaturedInsights**   | Insights teaser grid                                                          | `eyebrow`, `title`, `description`, `insights[]`, `ctas?`             | _Not mounted_ (available for future home layout)     |
| **LeadMagnetSection**  | PDF/download CTA block                                                        | `title`, `description`, `downloadHref`, `ctaLabel`, `footnote?`      | _Not mounted_; CMS `LeadMagnet` exists in fallback   |
...
_Canonical path: `docs/Component_Map.md` (TASK-070)._
